%%
% Kan composition implies filling for the cartesian cubes
% November 2021
%%
...
\title{Notes on Kan composition versus Kan filling for cartesian cubical sets}
\author{Steve Awodey}
\maketitle
...
We first recall from \cite{awodey-qmsCC-git} the basic definitions of the \emph{fibration weak factorization system} for cartesian cubical sets, defined in terms of (partial) open box filling.  As a warm-up, we recall the specification from \cite{GS} for Dedekind cubes, where the interval $\I$ has connections, which we call ``biased filling''.  In \cite{AGH} it is shown that these fibrations agree with those specified in the ``logical style'' of \cite{CCHM,OP}.  We then define an ``unbiased'' version that results in more trivial cofibrations and is more appropriate to the cartesian setting.  

In the following, recall from \cite{awodey-qmsCC-git} that a \emph{$+$-algebra} is an algebra for the pointed polynomial endofunctor on $\cSet$ determined by the cofibration classifier $1\ra\Phi$, vis.\ $$X^+ = \sum_{\varphi:\Phi}X^{\{\varphi\}}\,,$$ and a map $Y\ra X$ is a (relative) $+$-algebra if it is a $+$-algebra in $\cSet/_{X}$ for the relative version of the same.  The relative $+$-algebras form the right class of the cofibration-trivial fibration (algebraic) weak factorization system. 
...
Recall that we are assuming the axioms for cofibrations:
\begin{enumerate}
\item[(C1)] All isomorphisms are cofibrations.
\item[(C2)] The composite of two cofibrations is a cofibration.
\item[(C3)] Cofibrations are monomorphisms.
...
In place of (C6), we could equivalently require the cofibrations to be closed under the join operation $A\vee B$ in the lattice of subobjects of an object (as is done in \cite{CCHM,OP}).
\medskip
...
The $\otimes\dashv\ \Rightarrow$ adjunction on the arrow category has the following useful relation to weak factorization systems (cf.~\cite{GS,Riehl,JT}). For any maps $a : A \ra B$ and $f : X\ra Y$ we write 
\[
a\, \pitchfork\, f 
\]
...
%Moreover, for any $(x,y) : a \ra f$ as shown in \eqref{eq:defpitchfork}, we define the \emph{set}
%\[
%[a\, \pitchfork\, f]\ := \{ j : B \ra X\ |\ f\circ j = j\circ 
%\]
...
there is a logical equivalence between the diagonal filling \emph{conditions},
\[
(a\otimes b)\, \pitchfork\, c\  \Leftrightarrow\ a\, \pitchfork\, (b\Rightarrow\! c).
\]
Moreover, there is a natural isomorphism of the associated \emph{sets of witnesses},
\[
\{j : (a\otimes b)\, \pitchfork\, c\}\  \cong\ \{k : a\, \pitchfork\, b\Rightarrow\! c\}\,,
\]
which we will not spell out (cf.~\cite{Riehl})
\end{lemma}
%%%
...
\end{equation}
This is a reformulation of the logical specification given in \cite{CCHM,OP} (cf.~\cite{AGH}).

\begin{definition}\label{def:uniform} A \emph{(uniform, biased) fibration structure} on a map $f: Y\ra X$ is a choice of fillers $j_\epsilon(c,x,y)$ as in \eqref{diagram:directedfillers} and satisfying \eqref{eq:uniformfillers} for all maps $u : \I^m\ra\I^n$.
\end{definition}
...
\item The canonical maps $$(\delta_\epsilon\Rightarrow{X}) : X^I \ra X,$$ for $\epsilon = 0,1$, can be equipped with $+$-algebra structures.
...
\begin{proposition}
A map $f : Y\ra X$ is an (unbiased) fibration if and only if in the following diagram the canonical map $c$ to the pullback is a $+$-algebra.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Kan composition}\label{appendix:KanComp}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

A novelty of the type-theoretic notion of fibration is the method (due to Coquand and introduced in \cite{CCHM}) of reducing the (type-theoretically specified) notion of \emph{fibration structure} to the apparently weaker notion of a \emph{composition structure}.  Composition structure is more easily shown to be preserved by the type-forming operations like $\Sigma$ and $\Pi$ (see \cite{CCHM, OP}). We give a reformulation into diagrammatic language of those type-theoretic definitions and constructions.
Again we start with the case of an interval $\I$ with connections.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Composition with connections}\label{appendix:comptofillconnection}

\begin{definition}\label{def:composition}
For points $p, q: 1 \ra \I$, a cubical set $X$ \emph{has composition from $p$ to $q$} if for every object $Z$ and cofibration $c:C\mono Z$ and commutative square 
...
\end{equation}
where $\epsilon_p = X^p : X^\I\ra X$ is the \emph{evaluation at $p$} map, 
...
\end{equation}
where $\epsilon_q : X^\I\ra X$ is evaluation at $q$.  
\end{definition}

\begin{proposition}
In cubical sets with connections, if an object $X$ has composition from $\delta_0$ to $\delta_1$ and back, then $X$ has filling for all trivial cofibrations $c\otimes \delta : B \mono Z\times \I$, where $c : C\mono Z$ is any cofibration and $\delta = \delta_0 , \delta_1 : 1\ra \I$. %Thus $X$ is fibrant.  The same holds for maps $f: Y\ra X$.
\end{proposition}
...
Conversely, we can obtain filling for e.g.\  open 2-boxes from composition as follows: 
...
\]
First make a higher-dimensional composition problem using the connections on the right and left sides, and identities on the front and bottom: 
\[
\xymatrix@=1em{
A' &&& B'  \\
...
\]
Then since $X$ has composition, the indicated (partial) open 3-box has a top face, which is then a filler for the original open 2-box.
\[
\xymatrix@=1em{
A' &&& B'  \\
 & A \ar[lu]_a \ar[r]^p & B \ar[ru]^b & \\
...
If $X$ is fibrant in the \emph{unbiased} sense of section \ref{subsec:unbiasedfibration} then $X$ will have composition over $\I$ from the generic point $\delta : 1\ra \I$ to either of $\delta_0, \delta_1$, in the obvious sense, for the same reason as before: the map $(\delta\!\Rightarrow\!X) = X^\delta : X^\I \ra X$ is a trivial fibration over $\I$, by the definition of fibrancy. In order to arrive at a notion of composition equivalent to unbiased filling, we use a generic form of composition from $\delta$ to a second generic point $\delta'$.  This further point $\delta'$  is obtained by pulling back along the (say, second) projection $\pi : \I\times\I \ra \I$ to work in $\cSet/(\I\times\I)$, where in addition to $\delta, \delta_0, \delta_1$ we now also have a point $\delta' : 1 \ra \I$, given by the additional diagonal map over $\I\times \I$,
...
Then 
\begin{align*}
\eta_Y f = \eta_Y g\quad&\text{iff}\quad f^\Delta \eta_X= g^\Delta \eta_X : X\ra Y^\Delta\\
&\text{iff}\quad \Delta^*f = \Delta^*g : \Delta^*X \ra\Delta^*Y
\end{align*}
the latter by transposition, since $Y^\Delta = \Delta_*\Delta^* Y$. 
(A dual condition involving the counit $X\times \Delta \ra X$ also holds.)

We will define (unbiased) composition for an object $X$ by saying that, over $\I\times \I$, it has composition from $\delta$ to $\delta'$ in the sense of Definition \ref{def:composition}, but we need to be careful about what happens when $\delta$ and $\delta'$ ``coincide''.  Thus we require that the maps $k_1, k_2 : Z\rightrightarrows X$ where $\langle k_1, k_2 \rangle  = k : Z\ra X\times X$, are coequalized by the unit $\eta_X : X\ra X^\Delta$.  We reformulate this condition equivalently as follows.

\begin{definition}\label{def:unbiasedcomposition}
An object $X$ in $\cSet$ has \emph{(unbiased) composition} if in $\cSet/\I$ for every cofibration $c : C \mono Z$ and every commutative square of the form
...
\end{equation*}
upon pulling back along $\pi : \I\times\I \ra \I$ to $\cSet/\I\times\I$, there is a map $x' : Z \ra X$ making the following commute, 
...
\end{equation}
where $\epsilon_{\delta'} : X^\I \ra X$ is evaluation at the second generic point $\delta' : 1 \ra \I$.
...
\begin{proof}
One should first simplify the proof of Proposition \ref{prop:comptofillobj} before writing out the proof of Proposition \ref{prop:comptofillmap}.
\end{proof}

\begin{remark}
One can also promote the \emph{property} of an object or map ``having composition'' to that of a \emph{composition structure}, which is defined with respect to cofibrations $c : C\mono \I^n$ with representable codomains and a requirement of uniformity in $\I^n$.
\end{remark}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Fibrancy of $\U$}

An example of the use of composition rather than filling is the following proof of fibrancy of the universe (due to Coquand).  It uses the version of composition for cubes with connections.  
...
\end{equation}
Since the projection $\mathsf{Eq} \ra \U$ is a trivial fibration by the equivalence extension property, there is a diagonal filler $j$ as indicated.  Composing then gives the required $k$.

The claimed map $i$ is the one known as $\mathsf{IdtoEq}: \U^\I \ra \mathsf{Eq}$, and is defined in type theory by path induction, which apparently requires $\U^\I \ra \U\times\U$ to be a fibration.  It can also be defined explicitly, however, using the classifying property of $\U$.
\end{proof}
