...
Fix a small index category $\C$.  For a presheaf $X$ on $\C$, recall that the category of elements is the comma category,
\[\textstyle
\int_\C X\ =\ \yon_\C/_X\,,
\] 
where $\yon_\C : \C \to \psh\C$ is the Yoneda embedding, which we may omit and write simply $\C/_X$. 
...
\]
 ...
 \widehat\C\big( \y{c}\,,\ \nu_\C(\A) \big)\ \cong\ \nu_\C(\A)(c)\  =\ \Cat\big( {\C/_c}\,,\ \A \big)\ \cong\ \Cat\big( \int_\C\y{c},\ \A \big)\,.
  \]
For arbitrary presheaves $X$, one can use the presentation of $X$ as a colimit of representables over the index category over $\int_\C X$, and the easy to prove fact that $\int_\C$ itself preserves colimits.  Indeed, for any category $\D$, we have an isomorphism in $\Cat$,
\[
...
\end{align*}
 are as follows.  At $c\in\C$, for $x : \y{c}\ra X$, the functor $\eta_c(x) : \C/_c \to \C/_X$ is just composition with $x$, which may be written
\[
\eta_c(x) = \C/_x : \C/_c \to \C/_X\,.
\]
For $\A$, the functor $ \epsilon : \int\nu\A \to \A$ takes a pair $(c, f : \C/_c \to \A)$ to the object $f(1_c) \in \A$,
\[
\epsilon(c,f) = f(1_c).
\]
\begin{lemma}
...
Note that for the terminal presheaf $1\in\widehat{\C}$ we have $\int 1 \cong\C$, so for every $X\in\widehat{\C}$ there is a canonical projection  $\int X \to\C$, which is easily seen to be a discrete fibration. Ignoring size issues for the moment, discrete fibrations  in $\Cat$ are classified by the forgetful functor $\op{\dot{\Set}}\to \op{\Set}$ from (the opposites of) the category of pointed sets to that of sets.  For every presheaf $X\in\widehat{\C}$, we therefore have a pullback diagram
\[\textstyle
\begin{tikzcd}
	 \int X \ar[d] \ar[r] \pbmark & \op{\dot{\Set}} \ar[d]\\  
	\C \ar[r,swap,"X"] &  \op{\Set}.
 \end{tikzcd}
 \]
 Transposing by the adjunction $\int\dashv \nu$ then gives a commutative square in $\widehat{\C}$,
\begin{equation}\label{eq:classifyuniverse}\textstyle
\begin{tikzcd}
	 X \ar[d] \ar[r] & \nu\op{\dot{\Set}} \ar[d]\\  
	1 \ar[r,swap,"\tilde{X}"] &  \nu\op{\Set}.
 \end{tikzcd}
 \end{equation}
Let us write $\VV \to \V$ for the map on the right, so that
...
\begin{lemma}
The square \eqref{eq:classifyuniverse} is a pullback in $\widehat{\C}$. More generally, for any map $X\ra Y$ in $\widehat{\C}$, there is a pullback square 
\begin{equation}\label{eq:classifyuniverse}\textstyle
\begin{tikzcd}
	 X \ar[d] \pbmark \ar[r] & \nu\op{\dot{\Set}} \ar[d] \ar[r, "{=}"] & \VV \ar[d]\\  
	Y \ar[r] &  \nu\op{\Set}\ar[r,swap, "{=}"] & \V.
 \end{tikzcd}
 \end{equation}
\end{lemma}
