...
\end{equation}
with a generating trivial cofibration on the left, there is a diagonal filler $j$ as indicated.

Fixing any map $u : A \to B$, the pushout-product is a functor on the arrow category 
\[
(-)\!\otimes u : \cSet^\mathbbm{2} \to\cSet^\mathbbm{2}\,.
\]
This functor has a right adjoint, the pullback-hom (or ``Leibniz exponential''), which for a map $f : X\to Y$ we will write as
\[
