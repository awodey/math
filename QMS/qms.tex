...
\end{equation}
where $\epsilon = X^{\delta}$, $A_\epsilon = X^\I \times_{X} A$, and the struck-through arrow indicates a +-algebra.
...
\begin{proof}
We have the following for the fibration structures on $B\ra A$ and $A\ra X$.
...
%\end{equation}
\xymatrix{
B^\I \ar[r]|+  & B_{\epsilon_A} \ar[rr]  \ar[d] \pbcorner & & B \ar[d]\\
&A^\I \ar[r]|+ & A_{\epsilon} \ar[d] \pbcorner \ar[r] & A \ar[d]\\
&& X^\I \ar[r]_{\epsilon} &  X,
}
\end{equation}
...
\]
Let us regard $\delta\Rightarrow\alpha\beta$ as a map in the slice category over $A_\epsilon$, with the evident domain $\beta_\epsilon\circ (\delta\Rightarrow\alpha\beta)$.  Applying the pushforward along $\alpha_\epsilon$ then gives the map $\delta\Rightarrow\alpha_*\beta$, as can be checked by a calculation in extensional dependent type theory as the internal language of LCCCs.  Now we use the fact that pushforward preserves +-algebras, which can be shown directly, or by by observing that its left adjoint preserves cofibrations, to infer that $\delta\Rightarrow\alpha_*\beta$ is indeed a +-algebra, as required.
\end{proof}
