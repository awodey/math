...
\]
so there is a map over $X^\I$  (the image under $\Pi_{A_\epsilon}$ of the unit $B_\epsilon \ra p_*p^* B_\epsilon$):
\begin{equation}\label{diagram:frobenius3}
\xymatrix{
& X^\I  \\
...
\end{equation}
Our objective is now to fill in the further arrows $\varphi, \psi,\vartheta$ indicated below in order to get a retraction in the arrow category over $X^\I$.
...
\noindent For $\vartheta$: referring to the diagram \eqref{diagram:frobenius2}, since $p : A^\I \ra A_\epsilon$ is a trivial fibration, it has a section $o :  A_\epsilon \ra A^\I$.  Pulling back $p^*B_\epsilon \ra A^\I$ along $o$ results in an iso over~$A_\epsilon$,
\[
o^*p^* B_\epsilon \cong B_\epsilon \,,
\]
and so by adjointness there is a map over $A^\I$,
\[
...
\]
which is a retraction of the unit $B_\epsilon \ra p_*p^* B_\epsilon$ over $A_\epsilon$.  Applying the functor $ \Pi_{A_\epsilon}$ gives the desired retraction $\vartheta$.

\medskip
\noindent For $\psi$, we require a map 
\[
...
\]
where $\I^*(\Pi_{A^\I}\,B^\I)$ is over $\I^*X$ by composition with the counit $$\varepsilon: \I^*(X^\I)\cong \I^*\I_*\I^*X \to \I^*X.$$ 
Thus, dropping the $\I^*$ everywhere for readability (and since $\I^*$ preserves all the operations involved), it suffices to give
\begin{equation}\label{eq:needforpsi}
\Pi_{A^\I}B^\I \to (\Pi_AB)_\varepsilon \qquad \text{over $X^\I$.}
\end{equation}
We are thus in a situation very similar to diagram \ref{diagram:frobenius2},  
\begin{equation}\label{diagram:frobenius4}
\xymatrix{
...
\end{equation}
except that in the (here implicit) further slice over $\I$, the counit $\varepsilon: \I^*(X^\I) \ra \I^*X$ agrees with the evaluation at the (new) generic point $\delta': 1\ra \I$.  
Observe that in fact $\delta' = \I^*\delta$, so that under $\I^*$ the former $\epsilon = X^\delta : X^\I \ra X$ in \eqref{diagram:frobenius2} becomes the counit $\varepsilon : X^\I \ra X$.
...
\[
\Pi_{A^\I}B^\I \cong \Pi_{A_\varepsilon} p_*B^\I \to \Pi_{A_\varepsilon}B_\varepsilon  \qquad \text{over $X^\I$}
\]
as required in \eqref{eq:needforpsi}.

\medskip
\noindent For $\varphi$, we require a map 
\[
...
\end{equation}
Any map $i$ induces a unique map $ii$, which in turn determines a unique map $iii$.  We now apply $(-)^\I$ to the two righthand columns and interpolate the result on the left, using the unit $Z \ra (Z\times \I)^\I$.
...
\]  
Thus there is an associated map $v$, which, when precomposed with the unit $Z \ra (Z\times \I)^\I$ gives the desired map $Z \to \Pi_{A^\I}B^\I$.
