...
\end{lemma}
\begin{proof}
A fibration weak equivalence $f : Y\epi X$ is a trivial fibration, and therefore has a section $s: X\to Y$ (by lifting against the cofibration $0\mono X$.  Moreover, there is a homotopy $\vartheta : sf \sim 1_Y$, resulting from the lifting problem

???2
\begin{equation*}%\label{diagram:extensionforX}
%\xymatrix{
%Y+Y \ar@{>->}[d]_{} \ar[r]^{}  & Y^\I \ar@{>>}[d]\\
%Y \ar[r]_{\langle sf,1_Y\rangle} & Y\times Y.
%}
\end{equation*}
Thus $f$ is a homotopy equivalence, and so a WHE by lemma \ref{HEisWHE}.
\end{proof}
...
\begin{proof}
Let the cofibration $c : A \mono B$ be a WE, so a trivial cofibration. 
...
If $K$ is fibrant, any cofibration $c : A \mono K$ that is a WHE is a WE.
\end{lemma}

\begin{proof}
Let the cofibration $c : A \mono K$ be a WHE.
...
$i$ and $p$ are WHE, whence so it $f$.  Conversely, let $f$ be a WHE and factor it into a trivial cofibration $i : X\mono Z$ followed by a fibration $p: Z\epi Y$. Since  $i$ is then a WHE, as is $f$, it follows that $p$ is as well.  Thus $p$ is also a WE, hence a trivial fibration. So $f$ is a WE.
\end{proof}
