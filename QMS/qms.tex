...
\end{lemma}
\begin{proof}
This is clear, since fibrations are the right class of a weak factorization system, but let us see how the ``algebraic'' specification \eqref{diagram:frobenius1} is also stable under under pullback.
\end{proof}
...
\]
which is a +-algebra, because these are preserved by pushforward by lemma \ref{lemma:pluspushforward}.
...
\noindent $\bullet$ For $\vartheta$: referring to the diagram \eqref{diagram:frobenius2}, since $p : A^\I \ra A_\epsilon$ is a trivial fibration, it has a section $o :  A_\epsilon \ra A^\I$ by lemma \ref{lemma:trivfibsect}.  Pulling  $p^*B_\epsilon \ra A^\I$ back along $o$ results in an iso over~$A_\epsilon$,
\[
...
\]
This is easily seen to be a retraction of the unit $B_\epsilon \ra p_*p^* B_\epsilon$ over $A_\epsilon$.  Applying the functor $ \Pi_{A_\epsilon}$ gives the desired retraction $\vartheta :  \Pi_{A_\epsilon}p_*p^* B_\epsilon \ra \Pi_{A_\epsilon}B_\epsilon $ of $s$.
