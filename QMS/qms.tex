...
In this section, we will show that the fibration WFS from section \ref{FWFS} has the \emph{Frobenius property}: the left maps are stable under pullback along the right maps (see \cite{vdBG}).  This will imply the \emph{right properness} of our model structure: the weak equivalences are preserved by pullback along fibrations.  The Frobenius property is also needed in the proof of the equivalence extension property in the next section.  A proof of Frobenius in a related setting of cubical sets with connections can be found in \cite{GS}; however the type theoretic approach of \cite{OP,CCHM} provides an alternate route that is perhaps more direct, and in any event can also be applied without connections.  This approach proves the ``dual'' fact that the right adjoint to pullback (which always exists in a topos) along any \emph{fibration} $f : Y\ra X$ preserves fibrations.  This corresponds to the type-theoretic $\Pi$-formation rule.  

A novel aspect of this approach is the method (due to Coquand and first introduced in \cite{CCHM}) of reducing the (type-theoretically specified) notion of \emph{fibration structure} to the apparently weaker notion of a \emph{composition structure}.  Composition structure is more easily shown to be preserved by the type-forming operations like Sigma and Pi, when these concepts are formulated in type theory, as is done in \cite{CCHM}, or in the internal lanuage of the ambient presheaf topos, as in \cite{OP}.  This is due to the fact that one can then efficiently calculate using the rules of type theory (and even in a proof assistant), making it possible to prove e.g.\  that the fibrations are closed under Pi-types.  The approach taken here is a reformulation into categorical terms of those type-theoretic calculations.

\subsection{Composition and filling}

Let $p : 1 \to \I$ be any point of the interval (e.g.\ $\delta_0$), and $\epsilon_p := X^p : X^\I \to X$ the corresponding ``evaluation at $p$'' map.  Given another point $q : 1 \to \I$, there is an evident factorization 
\[
...
If $X$ is fibrant in the biased sense of section \ref{sec:biasedfibration}, then $X$ clearly has composition from $\delta_0$ to $\delta_1$ (and back), since the outer rectangle then has a diagonal filler (as does the corresponding one with $\pi_2$ for $\pi_1$).  One can show the converse, that having composition implies filling for all such squares \eqref{diagram:coposition1}, in the case where the category of cubes is assumed to have connections.  Logical proofs of this fact can be found in \cite{CCHM,OP}; a diagrammatic proof analogous to the following one for the unbiased case can also be given, but is omitted here.

If $X$ is fibrant in the unbiased sense of section \ref{sec:unbiasedfibration}, using the generic point $\delta : 1\ra \I$, then over $\I$ it will also have composition from any of $\delta, \delta_0, \delta_1$ to any other one, for the same reason as before: the map $(\delta\!\Rightarrow\!X) = X^\delta : X^\I \to X$ is a trivial cofibration there. In order to arrive at a property equivalent to unbiased filling, however, we use the following generic form of composition, which implies the various special cases. To formulate it, we introduce a second generic point by pulling back along the (say, first) projection $\pi : \I\times\I \ra \I$ to work in $\cSet/(\I\times\I)$, where in addition to $\delta, \delta_0, \delta_1$ we also have a second generic point $\delta' : 1 \ra \I$.

\begin{definition}\label{def:unbiasedcomposition}
An object $X$ will be said to have \emph{(unbiased) composition} if it has composition, in the sense of \eqref{diagram:composition2}, from $\delta$ to $\delta'$ over $\I\times \I$.
\end{definition}
...
\begin{proof}
Let $X$ be a cubical set with composition from  $\delta$ to $\delta'$ over $\I\times \I$, and suppose given a filling problem in $\cSet/\I$  of the form
...
\end{equation}
Pulling back along $\pi : \I\times\I \ra \I$ to $\cSet/(\I\times\I)$, since $X$ has composition there is then a map $k : Z\to X\times X$ over $\I\times\I$ as indicated below.
\begin{equation}\label{diagram:compimpfill2}
\xymatrix{
C \ar@{>->}[dd]_{c} \ar[r]^x  & X^\I \ar[d]^{\langle \epsilon_\delta, \epsilon_{\delta'}\rangle} \\
& X\times X \ar[d]^{\pi_1} \\
Z \ar[r]_y \ar@{..>}[ru]^{k} & X,
}
\end{equation}
...
\xymatrix{
C \ar@{>->}[dd]_{c} \ar[r]^x  & X^\I \ar[dd]_{\epsilon_\delta} \ar[r]^{\rho_{X^\I}}
	&  (X^{\I})^{\I} \ar[d]^{\pi_*\langle \epsilon_\delta, \epsilon_{\delta'}\rangle} \\
 & & (X\times X)^\I \ar[d]^{\pi_1^\I} \\
Z \ar[r]_y \ar@{..>}[rru]^>>>>>>>>>{k} & X\ar[r]_{\rho_X} & X^I,
}
\end{equation}
where $\rho_X : X\ra X^\I$ is the unit of $\pi^* \dashv {\pi}_*$, and similarly for $\rho_{X^\I}$.   

Next observe that the maps $X^\delta : X^\I \ra X$ and $(X^\I)^\delta : (X^{\I})^{\I} \ra X^\I$ make the following commute, and indeed make $\epsilon_\delta$ a retract of $\pi_1\circ\pi_*\langle \epsilon_\delta, \epsilon_{\delta'}\rangle$ in the arrow category of $\cSet/\I$.
...
XXXXX Need to add the side condition on the composiiton when the two points are the same.
...
\begin{remark}
There is an enitirely analogous generalization, the details of which we omit, from compositon for an object $X$ to composition for a map $f : Y\to X$, just as for the notion of (trivial) fibration.  Also analogous is the promotion of the \emph{property} of an object or map ``having composition'' to the notion of a \emph{composition structure}.  This procedes via the notion of a \emph{uniform composition structure} with respect to cofibrations $c : C\mono \I^n$, with representable codomains, which can then be internalized as a suitable map representing the uniform structure, in a way that is entirely analogous to the case for trivial fibrations formulated in proposition \ref{prop:uniformstructequivrelative}.
\end{remark}
