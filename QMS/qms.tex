...
A \emph{generating class of trivial cofibrations} are all maps of the form
...
%\end{proposition}
The \emph{fibrations} are defined to be the right class of these generating trivial cofibrations,
\[
(\mathcal{C}\otimes \delta_\epsilon)^\pitchfork\ =\ \mathcal{F}\,.
\]
Thus a map $f : Y\to X$ is a fibration if for every commutative square of the form
...
\paragraph{Fibration structure.} For any fibration $f:Y\to X$ a (uniform) fibration structure on $f$ is a choice of diagonal fillers $j(c,\epsilon)$,
...
\begin{definition} A \emph{(uniform) fibration structure} on a map $f: Y\to X$ is a choice of fillers $j(c,\epsilon)$ as in \eqref{diagram:directedfillers} satisfying \eqref{eq:uniformfillers} for all $u : \I^m\to\I^n$.
\end{definition}

Clearly, any map $f: Y\to X$  that can be equipped with fibration structure is a fibration in the sense of \eqref{diagram:biasedfillers}.  We shall assume that every fibration comes with a chosen fibration structure. (See \cite{GS} for a proof that such a choice can be made.)  Using the $\otimes\dashv\ \Rightarrow$ adjunction on the arrow category, we can now show the following (cf.~\cite{GG}, prop.~n.m).
...
\[
(c\otimes\delta_\epsilon)\, \pitchfork\, f \qquad\text{(for all $c\in \mathcal{C}$)}.
\]
By the $\otimes\dashv\ \Rightarrow$ adjunction, this is equivalent to the condition  
\[
c\, \pitchfork\, (\delta_\epsilon\!\Rightarrow\! f) \qquad\text{(for all $c\in \mathcal{C}$)}.
\]
...
\end{equation*}
with an arbitrary cofibration $c : C \mono Z$ on the left, has is a diagonal filler $k$ as indicated. 
But this is just to say that the map $\delta_\epsilon\Rightarrow{f}$ is in the right class of the cofibrations, which is equivalent to being a $+$-algebra, as claimed.
\end{proof}
...
Rather than building a weak factorization system based on the foregoing notion of fibration (as is done in \cite{GG,OP}), we shall first eliminate the ``bias'' on a choice of endpoint $\delta_\epsilon : 1 \to \I$, expressed by the indexing $\epsilon = 0,1$.  Consider first the ``path-lifting'' condition, which is a simple special case of \eqref{diagram:biasedfillers} with $c =\, ! : 0\mono 1$, since $!\otimes\delta_\epsilon = \delta_\epsilon$:
\begin{equation*}
\xymatrix{
...
Now we can play the proof of Proposition \ref{prop:algequivfill} backwards in order to determine a class of generating trivial cofibrations for the unbiased case. We consider pairs of maps $z:Z\to\I$ and $c : C\mono Z$, where the former is regarded as an ``indexing'' and the latter is a cofibration, thus
\[
\xymatrix{
C \ar[rd]_{zc} \ar@{>->}[r]^{c} & Z  \ar[d]^z \\
& \I
}
\]
can be regarded as an $\I$-indexed family of cofibrations.
