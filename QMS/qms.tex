...
If $X$ is fibrant in the biased sense of section \ref{sec:biasedfibration}, then $X$ clearly has composition from $\delta_0$ to $\delta_1$ (and back), since the outer rectangle then has a diagonal filler (as does the corresponding one with $\pi_2$ for $\pi_1$).  One can show the converse, that having composition implies filling for all such squares \eqref{diagram:coposition1}, in the case where the category of cubes is assumed to have connections.  Logical proofs of this fact can be found in \cite{CCHM,OP}; a diagrammatic proof is given in an appendix 1.

If $X$ is fibrant in the unbiased sense of section \ref{sec:unbiasedfibration} with the generic point $\delta : 1\ra \I$ over $\I$, then $X$ will have composition from any of $\delta, \delta_0, \delta_1$ to any other one, for the same reason as before: the map $(\delta\!\Rightarrow\!X) = X^\delta : X^\I \to X$ is a trivial fibration over $\I$. In order to arrive at a property equivalent to unbiased filling, however, we use a generic form of composition that implies the various special cases. To formulate it, we introduce a second generic point by pulling back along the (say, second) projection $\pi : \I\times\I \ra \I$ to work in $\cSet/(\I\times\I)$, where in addition to $\delta, \delta_0, \delta_1$ we now also have a second generic point $\delta' : 1 \ra \I$, given by the additional diagonal map over $\I\times \I$,
...
Observe that in $\cSet/\I\times\I$ the diagonal $\Delta : \I\mono \I\times\I$ is a subobject of the terminal object $\Delta\mono 1$, with associated base change 
...
Given any objects and arrows $f, g : Y \rightrightarrows X$ in $\cSet/\I\times\I$,
we have $\Delta^*f = \Delta^*g$ in $\cSet/\I$ if and only if the composites with $\eta_X$ are equal,
\begin{equation}\label{diagram:composeDelta}
\xymatrix{
Y \ar@<-1ex>[r]_{g}  \ar@<1ex>[r]^{f} & X \ar[r]_{\eta_X} & X^\Delta\,.
}
\end{equation}
\end{lemma}

We can now define what it means for an object $X$ to have (unbiased) composition by saying that it has composition, in the sense of \eqref{diagram:composition2}, from $\delta$ to $\delta'$ over $\I\times \I$, and that, moreover, the two resulting maps $k_1, k_2 : Z\rightrightarrows X$ are coequalized by the unit $\eta_X : X\ra X^\Delta$.  We reformulate this condition equivalently as follows
...
\end{equation}
Finally, we claim that $\pi_*X^\Delta \cong X$, and that, up to this iso,  $$\pi_*\eta_X = \epsilon_\delta : X^\I\ra X,$$ which will finish the proof, since this is a retraction of $\eta^\pi_X : X\ra X^\I$.  Indeed, writing out the object $\pi_*X^\Delta$ in terms of the two adjunctions $\pi^* \dashv {\pi}_*$ and $\Delta^* \dashv \Delta_*$ we have
\[
...
\]  
By a triangle law, this map has the inverse $\I^*(\eta_{X}^\I) : X\times\I \to X^\I \times \I$, where $$\eta_X^\I : X \ra X^\I$$ is the unit of $\I^*\dashv \I_*$ at $X$.  It suffices to show that $\I^*(\eta_{X}^\I)$ is also an inverse for $\pi_*\eta_X$ which, more explicitely is:
\[
\pi_*\eta_X = \pi_*(\eta^\Delta_{\pi^*(\I^*X)}).
\]
Since $\I^*$ preserves exponentials, we have
\[
\I^*(\eta_{X}^\I) = \eta_{(\I^*X)}^\pi : \I^*X \to (\I^*X)^{\I^*\I} \cong \pi_*\pi^*(\I^*X)\quad\text{(over $\I$)}.
\]
Now, for the composition of the $\pi^* \dashv {\pi}_*$ and $\Delta^* \dashv \Delta_*$ adjunctions, the units $\eta^\pi$ and $\eta^\Delta$ satisfy the law
\[
