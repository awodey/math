...
\item A fibration structure on $p_A : A\ra X$ is then a relative +-algebra structure on $\delta\Rightarrow p_A$ in the slice category over the codomain. We continue further to construct the object $\Fib(A)\ra X$
...
\item A +-algebra structure on $\epsilon^A$ is a retract $\alpha$ of the unit $\eta$ as indicated below, where $D$ is the domain of the map $(\epsilon^A)^+$:
...
\begin{proposition}
The map $\FFib\ra\Fib$ just constructed is has a canonical fibration structure.
\end{proposition}
\begin{proof}
