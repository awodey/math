...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Partial box filling (biased)}\label{sec:biasedfibration}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Partial box filling (unbiased)}\label{sec:unbiasedfibration}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\subsection{Classifying equivariant fibrations}
...
\end{equation}
Here $p_0, p_1$ are the evaluations $Y^{\delta_{0}}, Y^{\delta_{1}}$ at the endpoints $\delta_0, \delta_1 : 1\to\I$, and let $r:= Y^!$ for $! : \I\to 1$.   Then let $P_f:= f^*(Y^\I)$ and $p_f : f^*p_0$.  Let $s:= f^*r : X\to P_f$ (over $X$), and $t:= p_1\circ p_0^*f : P_f \to Y$, the horizontal composite across the middle of the diagram. 
...
& \ar[ld]_t P_f  \ar[d]^{p'} \ar[rr]^{p_0^*f} \pbcorner && Y^\I \ar[d]_{p} \ar[rd]^{p_1}\\
Y & \ar[l]^-{\pi'_1} X\times Y\ar[d]^-{\pi'_0} \ar[rr]_{(f,1_Y)} \pbcorner  &&  Y\times Y \ar[d]_{\pi_0} \ar[r]_-{\pi_1} & Y \\
& X \ar[rr]_f  &&  Y &
}
\end{equation}
The second factor $t = p_1\circ p_0^*f : P_f \to Y$ now appears also as $\pi_1\circ(f,1_Y)\circ p'$, which is $p': P_f \to X\times Y$ followed by the second projection $\pi'_1 : X\times Y \to Y$ (which is not a pullback). But if $Y$ is fibrant, then $p : Y^\I \to Y\times Y$ is a fibration, and then so is $p'$. And if $X$ is fibrant, then the projection $\pi'_1:X\times Y \to Y$ is a fibration. Thus in this case, $t = \pi'_1\circ p' : P_f \to Y$ is a fibration, as claimed.
...
\end{equation}Note that the retraction $p_f : P_f \to X$ is \emph{not} over $Y$.
\end{enumerate}

The following lemma will also be needed. Note that it concerns only the cofibration weak factorization system studied in section \ref{sec:cofibrations}.
...
\begin{proof}
The map $\eta:E \to c_*c^*E$ is the pullback-hom $c\!\Rightarrow\! p$, as is easily checked.
...
Now let $w=t\circ s$ be the pathspace factorization \eqref{diag:pathspace factorization} of $w$, as a map over $C$.  Since $q:B \to C$ is a fibration, by the foregoing remarks on pathspace factorizations, we know that $s : A\to P_w$ has a retraction $p_w : P_w \to A$ which is a trivial fibration.  The retraction $p_w$ is a map over $C$, but not over $B$.  

The pathspace factorization $w=t\circ s : A \to P_w \to B$ is stable under pullback along $c$, providing a pathspace factorization $w'=t'\circ s' : A' \to P_{w'} \to B'$ over $C'$.  Since both $p'$ and $q'$ are fibrations, $s' : A' \to P_{w'}$ has a retract $p_{w'}: P_{w'} \to A'$ that is a trivial fibration, and now $t' : P_{w'} \to B'$ is a fibration, since $q' : B' \to C'$ is a fibration.
...
To prove the remaining claim that $\eta : P_w \to  b_*b^*P_w$ is a trivial fibration, we want to use lemma \ref{lemma:etaTF}, but to apply that directly we would need $t : P_w \to B$ to be a trivial fibration, which we do not have at that point.  Instead, we will show that $\eta$ is a pullback of the corresponding unit at the trivial fibration $p_1 : B^\I \to B$.
...
\end{equation}
The right hand face is a pullback by definition, and the remainder results from pulling the right face back along $b$. Thus all faces are pullbacks.  The base is also a pushforward, $w = b_*w'$, again by definition.  We claim the top is therefore also a pushforward, $p_0^*w = \overline{b}_*((p'_0)^*w')$. Indeed, since the front face is a pullback, the Beck-Chevalley condition applies and we have $\overline{b}_*(p'_0)^*(w') = p_0^*\,b_*(w') = p_0^*w$.

Now consider the following, in which the top square remains the same, but $p_0$ has been relaced by $p_1 : B^\I \to B$, so the composite at right is by definition $t = p_1\circ p_0^*w$.
...
\end{equation}
The horizontal direction is still pullback along $b$.  Let us rename $u := p_0^*w$ so that $(p'_0)^*w' = b^*u$ and $t' = b^*t$ and $p'_1 = b^*p_1$ to make this clear, and we then add the pushforward along $b$ on the right, in order to obtain the two units $\eta$.
...
Returning to the exact relation between the fibration extension property (Definition \ref{def:fibextreplace}) and the condition that the base object $\U$ is fibrant, we can easily see that the latter implies the former.  Indeed, let $t : X\cof X'$ be a trivial cofibration and $Y \fib X$ a fibration.  To extend $Y$ along $t$, take a classifying map $y : X \to \U$, so that $Y \cong y^*\UU$ over $X$. If $\U$ is fibrant then we can extend $y$ along $t : X\cof X'$ to get $y' : X' \to \U$ with $y = y'\circ t$.  The pullback $Y' = (y')^*\UU \fib X'$ is then a fibration such that $t^*Y' \cong t^*(y')^*\UU\cong y^*\UU \cong Y$ over $X$. 

Conversely, we remark that the FEP implies the fibrancy of $\U$, given the following ``alignment'' lemma, which will be required below (cf.(2') of~\cite{Shu:UAERP}).
...
We shall prove that $\U$ is fibrant using the equivalence extension property (section \ref{sec:EEP}).  This approach was first used in \cite{CCHM} via a different argument employing the reduction of filling to composition, which we do not require.
...
\xymatrix{
C \ar@{>->}[dd]_c \ar[rr]^{\tilde{a}} && \U^\I \ar[dd]^{\U^{\delta_0}} \\
&& \\
Z  \ar@{..>}[rruu] \ar[rr]_{b}  && \U  \\
}
\end{equation}
...
\xymatrix{
A_0 \ar@{>->}[dd] \ar@{>->}[rr] && A_0\times\I \ar@{>>}[dd] \\
&& \\
A  \ar@{>>}[rr] &&  C\times\I 
}
\end{equation}
...
\begin{equation}
\xymatrix{
A_0 \ar@{>->}[dd] \ar@{>->}[rr] \pbcorner && A_0\times\I \ar@{>>}[dd] && \ar[ll]_e A\\
&& \\
B  \ar@{>->}[rr]  &&  B\times\I  && \ar[ll]^f D
}
\end{equation}
