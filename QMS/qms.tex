...
\]
Let us regard $\delta\Rightarrow\alpha\beta$ as a map in the slice category over $A_\epsilon$, with the evident domain $\beta_\epsilon\circ (\delta\Rightarrow\alpha\beta)$.  Applying the pushforward along $\alpha_\epsilon$ then gives the map $\delta\Rightarrow\alpha_*\beta$, as can be checked e.g.\ by reasoning in extensional dependent type theory as an internal language for LCCCs.  But now we use the fact that pushforward preserves +-algebras, because its left adjoint preserves cofibrations, to infer that $\delta\Rightarrow\alpha_*\beta$ is indeed a +-algebra, as required.
\end{proof}
