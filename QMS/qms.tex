...
\begin{lemma}
For each $A : \I^n \ra \V$ there is a canonical choice of a small family $p_A : \overline{A}\ra \I^n$ and a map $q_A:\overline{A} \ra\VV$ making a pullback square as follows.
\begin{equation}\label{diagram:U2}
\xymatrix{
\overline{A} \ar[d]_{p_A} \ar[r]^{q_A} & \VV\ar[d]^{p}  \\
\I^n \ar[r]_{A} & \V
}
\end{equation}
\end{lemma}
%
\begin{proof}
Since $\I^n \cong y[n]$ is representable, there is a distinguished associated presheaf $A : (\C/[n])^{op} \ra \Set_\kappa$. Define $p_A : \overline{A} \ra \I^n$ by
\[
\overline{A}_k = \coprod_{h\in \C(k,n)} A(h) \qquad \ni (h, a)\,
\]
with first projection $(p_A)_k(h,a) = h$. Note that $\overline{A}_k$ is small.
Then let $q_A:\overline{A} \ra\VV$ be defined on $(h,a): \I^k \ra \overline{A}$ by 
\[
...
\xymatrix@=5em{
& \overline{A} \ar[d]^>>>>>>{p_A} \ar@{..>}[r]^{q_A} & \VV\ar[d]^{p}  \\
\I^k \ar[r]_{h} \ar[ru]^{(h,a)}\ar[rru]_>>>>>>>>>>>{a} & \I^n \ar[r]_{A} & \V
}
\end{equation}
The proof that it is a pullback is  left to the reader.
\end{proof}
...
\begin{equation}\label{diagram:U2}
\xymatrix{
E \ar[d]_{p_E} \ar[r]^{q_E} & \VV\ar[d]^{p}  \\
\I^n \ar[r]_{\chi_E} & \V
}
\end{equation}
\end{lemma}
...
\[
(\chi_E)_k(h) := \Gamma(h, E)\ = \ \{ s : \I^k \ra E \ |\ p_E\circ s = h \}.
\]
\[
\xymatrix{& E \ar[d]^{p_E} \\ \I^k \ar@{..>}[ru]^{s}\ar[r]_h & \I^n } 
\]
which is small if each $E_k$ is.  Define $q_E:E \ra\VV$, similarly, on $e : \I^k \ra E$ by first composing with $p_E$ and then observing that $e \in  \Gamma(p_Ee, E)$.  Since the assignment is clearly natural in $[k]$ it therefore gives a map $\dot{e} : \I^k \ra \VV$ making the following commute.
\begin{equation}\label{diagram:U4}
\xymatrix@=5em{
&E \ar[d]^>>>>>>>{p_E} \ar@{..>}[r]^{q_A} & \VV\ar[d]^{p}  \\
\I^k \ar[r]_{p_Ee} \ar[ru]^{e}\ar[rru]_>>>>>>>>>>>{\dot{e}} & \I^n \ar[r]_{E} & \V
}
\end{equation}
The proof that it is a pullback is again left to the reader.
\end{proof}

\begin{corollary}
Given a small family $p_E : E \ra \I^n$ there is a unique isomorphism $E \cong \overline{\chi_E}$ over $\I^n$ making a commutative diagram as follows.

\begin{equation}\label{diagram:U5}
\xymatrix@=5em{
E \ar[rd]_{p_E} \ar@/^2.0pc/[rr]^{q_E} \ar[r]^{\cong}& \overline{\chi_E} \ar[d]^{p_{\chi_E}} \ar[r]_{q_{\chi_E}} & \VV \ar[d]^{p}  \\
 &\I^n \ar[r]_{\chi_E} & \V
}
\end{equation}
\end{corollary}
...
\begin{proof}
Write $X = \varinjlim \I^n$ as a colimit of a cocone of maps $ x : \I^n \ra X$ from representables, over the canonical index category $([n],x) \in \int_\C X$.  Form the family of pullback squares below, where the dotted arrows represent cocones, and the one consisting of the $q_x : E_x \ra E$  is determined by taking the indicated pullbacks, and is therefore also a colimit. 
...
$$\overline{(-)} : \cSet(X, \V) \to \cSet_\kappa/X $$ as a left (quasi-)inverse $
E \cong \overline{\chi_E}$, which is (pseudo-)natural in $X$, 
but there is no corresponding uniqueness of classifying maps, relating $A : X\ra \V$ and $\chi_{\overline{A}} : X\ra\V$.  This is what is provided by the univalence of a universe $$p : \UU\ra\U$$ of \emph{fibrations}, which we now procede to construct.
\end{remark}
