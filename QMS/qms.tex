...
\begin{lemma}\label{lemma:fibextreplace}
GIven any fibration $ f : Y \mono X$ and a trivial cofibration $\eta: X\ra X'$ to a fibrant object $X'$,
...
A cofibration $ c : A \mono B$ is a WE, and thus a trivial cofibration, if it lifts against any fibration $f : Y\epi K$ with fibrant codomain.
\end{lemma}
...
The previous two lemmas \ref{lemma:CofWHEiffFibLift} and \ref{lemma:CofWEiffFibLift} now combine to give.
\begin{corollary}\label{cor:CofWHEtoWE}
A cofibration $ c : A \mono B$ that is a WHE is a WE and thus a trivial cofibration.
\end{corollary}
