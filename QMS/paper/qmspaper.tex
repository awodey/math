...
Note that the (valid) condition $b\circ c = a\circ \mathsf{Gph}(zc)$ turns this into the equivalent problem of extending the copair $[b,a]$ along the  (trivial cofibration) pushout-product $c\, \otimes_z \delta$ from \eqref{diagram:unbiasedpushoutproduct}, which we recall below for the reader's convenience.
%
...
\]
for the ``$\delta$-section'' of the cylinder over $C$, and let $A_\delta = (C_\delta)^*A \fib C$ be the ``section of $A$ over $C_\delta$''.  We then have $c^*B \cong A_\delta$ over $C$ by the outer square of \eqref{diag:Ufib1}.
\[
\xymatrix@=1em{
