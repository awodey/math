...
\begin{corollary}\label{cor:fibpathtriv}
Let $F\fib X$ be an unbiased fibration. Then the canonical endpoint projections $F^{\delta_\epsilon}: F^\I \to F$ over $X$ are trivial fibrations, for $\epsilon = 0,1$, where the (relative) pathobject $F^\I \to X$ is the exponential over $X$ of $F\to X$ by the pullback $X^*\I = \I \times X \to X$ of $\I$. 
%
\begin{equation}\label{diagram:unbiasedfibration}
\xymatrix{
F^\I \ar[rd] \ar[r]^{F^{\delta_\epsilon}} & F \ar@{>>}[d] & F^\I \ar[rd] \ar[r]^{F^{\delta}} & \I\times F \ar@{>>}[d] & F \ar@{>>}[d]  \\
& X \ar[d] \ar[rr]^{X^*\delta_\epsilon} \pbcorner && \I\times X \ar[d]  \ar[r] \pbcorner & X \ar[d]  \\
...
\begin{proof}
Pull $F\fib X$ back along $\I\to 1$ to obtain the fibration $\I^*F\fib \I^*X$ over $\I$.  we have $c \pitchfork F^{\delta_\epsilon}$ (over $X$). But since $F^{\delta_\epsilon} = (X^*\delta_\epsilon\!\Rightarrow\! F)$, this is equivalent to $c \otimes X^*\delta_\epsilon \pitchfork F$ (over $X$).
\end{proof}
...
The equivalence extension property (EEP) is closely related to the \emph{univalence} of the universal fibration $\UU\ra\U$ constructed in section \ref{sec:universalfibration} (see \cite{Shu2015}).  We shall use it in section \ref{sec:FEP} to show that the base object $\U$ of the universal fibration $\UU\fib\U$ is fibrant, which implies the fibration extension property.  The proof of the EEP given here is a reformulation of a type-theoretic argument due to Coquand \cite{CCHM}, which in turn is a modification of the original argument of Voevodsky \cite{KLV}.  See \cite{sattler} for another reformulation.  

Throughout this section we shall consider only the case of $\delta$-biased fibrations in $\cSet/_\I$. The EEP for this case will suffice to establish the model structures for both this case and for the unbiased fibrations in $\cSet$.  (The EEP for unbiased fibrations will then be shown by a different argument in Proposition \ref{prop:EECunbiased}).  Thus for the remained of this section, ``fibration'' always means $\delta$-biased fibration in $\EE=\cSet/_\I$, and  $\delta : 1 \to\I \in \EE$ is the diagonal $\I \to \I\times\I$ as a map over $\I$ in $\cSet$.
...
We make the following observations, which are well-known in general, but still need to be checked in this special case.
\begin{enumerate}
\item If $f:X\ra Y$ is a map over a base $Z$, we instead use the relative pathobject $Y^\I \to Z$, where the exponential is taken in the slice over $Z$, and the interval used is the result of pulling $\delta_0, \delta_1 : 1\ra\I$ back along $Z\to 1$. The factorization $t\circ s:X\ra P_f \ra Y$ is then stable under pullback along any map $g : Z' \ra Z$, in the sense that $g^*(Y^\I) \cong g^*(Y)^\I$ and so $g^*P_f = P_{g^*f}$, where $g^*f : g^*X \ra g^* Y$, and similarly for the factors $g^*s$ and~$g^*t$.  

\item The retraction $p_0\circ r = 1_Y$ pulls back along $f$ to a retraction $p_f \circ s =1_X$.

\item If $Y$ is a fibrant object, then $p_0 , p_1 : Y^\I \ra Y$ are both trivial fibrations, because ... \ednote{fix this} . If $f:X\ra Y$ is over a base $Z$ then ... \ednote{fix this too}  In that case, its pullback $p_f : P_f \ra X$ is also a trivial fibration. 
