...
This section applies the method of \emph{generalization} explained in the introduction, which procedes by first establishing a structure or proposition in the generic biased setting, and then transferring it to the unbiased setting by pullback along the base change $\cSet\to\cSet/_\I$.  In order to emphasize this method, we give the second step first, in the form of a conditional statement.
...
The results proved in this section will be applied to the slice category $\cSet/_\I$ and the generic point $\delta : 1 \ra \II = \I^*\I$, but nothing in their proofs depends on this particular case, and so we shall write simply $\delta : 1\to \I$ for a chosen pointed object in an arbitrary topos $\EE$.  (Indeed, in this section $\EE$ may even be taken to be just a locally cartesian closed category with a representable class of cofibrations.)

Recall from Definition \ref{def:unbiasedfibration} that a map $f:A\ra X$  is a $\delta$-biased fibration just if the map $\delta \Rightarrow f$ admits a relative +-algebra structure, and is therefore a trivial fibration.  The definition of the pullback-hom $\delta \Rightarrow f$ is recalled below.
...
From Proposition \ref{prop:biasedFrobimpliesunbiasedFrob} we therefore have:
...
In order to classify fibrations $A\fib X$, we shall proceed as for trivial fibrations by constructing an object $\Fib(A)\ra X$ of fibration structures on a map $A\ra X$ which, moreover, is stable under pullback.  We then apply the construction to the universal small family $\VV\ra\V$ of Proposition \ref{prop:familyclassifier} to obtain a universal small fibration.  

The construction of $\Fib(A)\ra X$ is a bit more involved than that of $\TFib(A)\ra X$.  Recall from section \ref{sec:unbiasedfibration} the characterization of (uniform, unbiased) fibration structures on a map $p_A : A\ra X$ in terms of +-algebra structures on $\delta\Rightarrow p_A$:  
\begin{enumerate}
...
\item A fibration structure on $p_A : A\ra X$ is then a relative +-algebra structure on the map $\delta\Rightarrow p_A$ in the slice category over the codomain $X^\I \times_X A$ (formed in the slice over $\I$). 
...
%\epsilon_A &:= A^\delta\\
\epsilon_A &:= \delta\Rightarrow p_A
\end{align*}
...
\item A +-algebra structure on $\epsilon_A$ is a retract $\alpha$ over $A_\epsilon$ of the unit $\eta$ as indicated below, where $D$ is the domain of the map $(\epsilon_A)^+$,
...
\item As in the construction \eqref{diag:plualgstru}, there is an object $+\mathsf{Alg}(\epsilon_A)$ over  $A_\epsilon$ of +-algebra structures on $\epsilon_A$, the sections of which correspond uniquely to +-algebra structures on $\epsilon_A$ (and thus fibration structures on $A$).
\begin{equation}\label{diagram:fibU4}
\xymatrix{
&A^\I \ar[d]_{\epsilon_A} \ar[r]_\eta & \ar@{.>}@/_1pc/[l]_\alpha D \ar[ld]^<<<<<{(\epsilon_A)^+}  \\
+\mathsf{Alg}(\epsilon_A) \ar[r] & A_\epsilon\ar[d]_{p_\epsilon} \ar[r] \pbcorner & A \ar[d]^{p} \\
& X^\I \ar[r]_{\epsilon} &  X 
}
\end{equation}

\item Sections of $+\mathsf{Alg}(\epsilon_A)\ra A_\epsilon$ then correspond to sections of its push-forward along $p_\epsilon$, which we shall call $F_A$: 
\[
F_A := (p_\epsilon)_*(+\mathsf{Alg}(\epsilon_A))\,.
\]
...
\item One might now think of taking another pushforward of $F_A \ra X^\I$ along $\epsilon : X^\I\ra X$ to get the object $\Fib(A) \ra X$ that we seek, but unfortunately, this would not be stable under pullback along arbitrary maps $Y\ra X$, because $\epsilon : X^\I\ra X$ is not stable in that way.  Instead we use the \emph{root} functor of Proposition \eqref{prop:Itiny}, i.e.\ the right adjoint of the pathspace, $(-)^\I \dashv (-)_\I$. (Recall that the interval $\I$ in the slice $\cSet/_\I$, namely $\I^*\I = \I\times\I\to\I$, is still tiny, by Lemma \ref{lemma:tinyslicedI}.)
...
\item\label{item:laststep} Finally, recall that we are still working in the slice $\cSet/_\I$ and need to get back to $\cSet$, which we will do by applying the pushforward $\I_*: \cSet/_\I \ra \cSet$.  Let us rename the map $\Fib(A)\ra X$ constructed over $\I$ in the last step to $\Fib_i(A) \ra \I^{*}X$, and then apply $\I_*$ to get the map,
\[
\I_*(\Fib_i(A)) = \Pi_{i:\I}\Fib_i(A) \ra X^\I
\]
...
\xymatrix{
 \Fib(A) \ar[d] \ar[r] \pbcorner & \ar[d] \Pi_{i:\I}\Fib_i(A) \\
 X \ar[r]_{\rho} &  X^\I
}
\end{equation}
It then follows directly from the adjunction $\I^*\dashv \I_*$ that sections of $\Fib(A)\ra X$ correspond bijectively to sections of $\Fib_i (A)\ra \I^{*}X$ over~$\I$.
\end{enumerate}

Now apply the foregoing construction to the universal family $\VV\to\V$ to get $\Fib(\VV) \ra \V$, and define the universal small fibration by setting $\Fib : =\Fib(\VV)$ and $\FFib\fib\Fib$ by pulling back the universal family,
...
\begin{proof}
First, we show that the construction of $\Fib(A)\ra X$ as the classifying type of fibration structures on a map $A\ra X$ is stable under pullback along all maps $f : Y\ra X$.
To that end, the relevant parts of the construction
%diagram \eqref{diagram:fibU5} 
given in steps (1)-(7) are recalled schematically below.
\begin{equation}\label{diagram:fibU8}
\xymatrix{
&A^\I \ar[d]_{\epsilon_A} & &\\
+\mathsf{Alg}(\epsilon_A) \ar[r] & A_\epsilon\ar[d]_{p_\epsilon} \ar[r] \pbcorner & A \ar[d]^{p} &\\
F_A \ar[r] & X^\I \ar[r]_{\epsilon} &  X & \ar[l] \Fib_i(A)
}
\end{equation}
...
\xymatrix{
 ...
 +\mathsf{Alg}(\epsilon_B) \ar[r] & B_\epsilon \ar[d] \ar[rd] \ar@{.>}[rrr] &&& A_\epsilon \ar[d] \ar[ld] &  +\mathsf{Alg}(\epsilon_A) \ar[l] \\
 F_B  \ar[r] & Y^\I \ar[rd] \ar@{.>}@/_.75pc/[rrr] & B \ar[r] \ar[d] & A \ar[d] &  X^\I \ar[ld]  & \ar[l] F_A \\
 &\Fib_i(B) \ar[r] & Y \ar[r]_f & X &\Fib_i(A) \ar[l] &
 }
\end{equation}
As in the proof of Lemma \ref{lemma:fibrationspullback}, on the left side we repeat the construction with $B \ra Y$ in place of $A\ra X$.  The left face of the indicated cube is then also a pullback, whence the back (dotted) face is a pullback, since the two-story square in back is the image of the front square under the right adjoint $(-)^\I$. Finally, the top rectangle in the back is therefore a pullback.
 
It follows that $+\mathsf{Alg}(\epsilon_B)$ is a pullback of $+\mathsf{Alg}(\epsilon_A)$ along the upper dotted arrow, as in diagram \eqref{diagram:pbplus}, and so the pushforward $F_B$ is a pullback of the corresponding $F_A$, along the lower dotted arrow (which is $f^\I$), by the Beck-Chevalley condition for the dotted pullback square.  Let us record this for later reference:
\begin{equation}\label{eq:pbFB}
F_B \cong (f^\I)^*F_A.
\end{equation}

It remains to show that $\Fib_i(B)$ is a pullback of $\Fib_i(A)$ along $f:Y\ra X$, and now it is good that we did not take these to be pushforwards of $F_B$ and $F_A$, because the floor of the cube need not be a pullback, and so the Beck-Chavalley condition would not apply.  Instead, consider the following diagram.
\begin{equation}\label{diagram:fibU10}
\xymatrix{
& \Fib_i(B) \ar[ld] \ar[dd]  \ar[r] & \Fib_i(A) \ar[dd] \ar[rd] & \\
(F_B)_\I \ar[dd] \ar[rrr] &&& (F_A)_\I \ar[dd] \\
& Y \ar[ld]_\eta \ar[r]_f & X \ar[rd]^\eta &\\
(Y^\I)_\I \ar[rrr]_{(f^\I)_\I} &&& (X^\I)_\I 
 }
\end{equation}
The sides of the cube are pullbacks by the construction of $\Fib_i(A)$ and $\Fib_i(B)$. The front face is the root of the pullback \eqref{eq:pbFB} and is thus also a pullback, since the root is a right adjoint. The base commutes by naturality of the unit of the adjunction, and so the back face is also a pullback, as required.  

Finally, the base change along $\I_* : \cSet/_\I \ra\cSet$ in step \ref{item:laststep} above clearly also preserves the pullback.

Thus we can indeed use $\Fib := \Fib(\VV)$ to define the universal small fibration $\FFib\ra\Fib$ as in \eqref{diagram:universalfib}, and the proof can conclude just as in that for proposition~\ref{prop:classTFib}.  
\end{proof}
...
Recall from Example \ref{universeexample:sliceuniverse} of Section \ref{sec:U} that the universe in the slice category $\cSet/_\I$ is the pullback of the universe $\V$ from $\cSet$ along the base change $\I^* : \cSet \to \cSet/_\I$.  Thus in Step \ref{item:laststep} above we are first building the classifying type $$\Fib_i(\I^*\VV) \to \I^*\V$$ for $\II$-fibration structures on the universal family in $\cSet/_\I$, and then taking a pushforward $\I_*: \cSet/_\I \to \cSet$ to obtain the classifier for fibrations as the pullback along the unit:
...
\end{equation}
We remark for later reference that this classifying type $\Fib = \Fib(\VV) \to \V$ for fibration structures can also be constructed as the pushforward of the classifier $\Fib_i(\I^*\VV) \to \I^*\V$ for $\II$-fibration structures, along the projection $q : \I^*\V =  \I\times \V \to \V$ indicated below.
