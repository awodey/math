...
\begin{enumerate}
\item Every trivial fibration $A\ra X$ has a section $s : X\ra A$, which is a cofibration.\ednote{add this!}
...
Our approach to proving that the classes $\CC$ of cofibrations from Section \ref{sec:cofibrations} and $\FF$ of fibrations from Section \ref{sec:fibrations} determine a model structure will be to first build a \emph{premodel structure} in the sense of \cite{Barton}, and then turn to the question of the 3-for-2 property for the weak equivalences.
...
\begin{proof}
Applying Lemma \ref{lem:WHEunwound}, condition (1) is immediate, and (2) results from the fact that $K^\I \fib K\times K$ is a fibration when $K$ is fibrant, since $\del : 1+1 \cof \I$ is a cofibration.

\ednote{show that $\del : 1+1 \cof \I$ is a cofibration!}
...
\end{lemma}
\begin{proof}
By Corollary \ref{cor:plusalgprops}, the trivial fibration $p : Y \onto X$ has a section $s: X\mono Y$. 

\ednote{show that sections are cofibrations!}
...
\end{equation*}
Thus $f$ is a homotopy equivalence, and therefore a weak homotopy equivalence by Lemma \ref{lemma:HEisWHE}.
\end{proof}
