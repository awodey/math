...
Note that the representables $\I^n$ are also closed under finite products, $\I^m \times \I^n = \I^{m+n}$.  We of course write $\I$ for $\I^1$ and $1$ for $\I^0$, which is terminal.  We will use the following basic fact about the cubes $\I^n$ in $\cSet$.
...
One can simply take as the cofibrations all monomorphisms in $\cSet$, but for some purposes (\eg\ \cite{}) it is convenient to know what is actually required of them. Thus to begin, the following axioms will be assumed.
...
\end{enumerate}
Further axioms for cofibrations will be added later as needed: two in Section \ref{sec:biasedfibration}, one in Section \ref{subsec:unbiasedfibration}, and a final one in Section \ref{sec:realignment} (see Appendix \ref{appendix:Cofibrations} for a summary).  %All monos are cofibrations just in case $\Phi = \Omega$.
...
The reader familiar with type theory will recognize the similarity to the ``partiality'' or ``lifting''  monad \cite{something}.  When all monos are cofibrations, so that $\Phi = \Omega$, this agrees with the partial map classifier $X^+ = \widetilde{X}$ from topos theory \cite{JohnstoneTT}.  We may therefore regard $X^+$ as the object of \emph{cofibrant partial elements} of $X$, as we now explain.
...
It now follows that a weak homotopy equivalence $f : X \to K$ with a fibrant codomain is a weak equivalence. To eliminate the condition on the codomain we shall use the following lemma due to \cite{Cisinski}.
...
\end{equation}
 is an equivalence over the base $\U$ via $p_2: \U^\I \to \U$, which is also a trivial fibration, because $\U$ is fibrant (by Proposition \ref{prop:Ufibrant}).  In type theory, the equivalence \eqref{eq:UA} can be expressed as $$(A=B)\simeq(A\simeq B)\,.$$\end{remark}
...
\addcontentsline{toc}{section}{B\quad Cubical sets as a classifying topos}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsection*{Cubical sets as a classifying topos}\label{subsec:classtopos}

Recall that the objects of the Cartesian cube category $\Box$ may be taken concretely to be finite, strictly bipointed sets, written $$[n] = \{\bot, x_1, ..., x_n, \top\},$$ and the arrows $f : [n] \to [m]$ to be all bipointed maps $[m]\to [n]$ (note the direction). 
The category of \emph{(Cartesian) cubical sets} is the presheaf topos
\[
\cSet =  \psh{\Box}.
\]
