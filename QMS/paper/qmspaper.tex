...
There are now many treatments of cubical sets in the literature, inclusing \cite{Grandis, Grandis, Jardine, Jardine, Bezem, OrtonPitts, Coquand, Cisinski}.  Our construction is intended to work in \emph{all} of these, insofar as the axioms in Definition \ref{axioms} below are satisfied.   
...
\end{split}
\]
Thus $X^\I$ is given by precomposition with the ``successor'' functor $\C\to\C$ with $[n] \mapsto [n+1]$. Precomposition always has a right adjoint, which in this case we write as $$X^\I\dashv X_\I $$ and calculate to be:
\begin{align*}\textstyle
X_\I(n) &\cong \hom(\I^n, X_\I) \\
&\cong \hom((\I^n)^\I, X) \\
&\cong \hom((\I^\I)^n, X) \\
&\cong \hom((\I+1)^n, X) \\
&\cong \hom\big(\sum_{k=0}^{n} \binom{n}{k}\I^{k}, X\big) \\
&\cong \prod_{k=0}^{n} X(k)^{\binom{n}{k}} ,
\end{align*}
using the fact that $\I^\I \cong (\I+1)$ as in \cite{Fiore}.
\end{proof}

\begin{remark}
size of the homsets in cartesian vs. other cubes.
fullness of the cubical nerve functor from Cat.
other stuff from old notes.
\end{remark}
...
\]
\item $X$ admits a \emph{uniform filling structure:} 
...
\]
\item $f:Y\ra  X$ admits a \emph{uniform filling structure}: 
...
\item If $f:Y\ra X$ is a trivial fibration and $g:Z\ra Y$ is a trivial fibration, then  $f\circ g:Z\ra X$ is a trivial fibration.
\item If $f:Y\ra X$ is a trivial fibration and $f':Y'\ra X'$ is a retract of $f$ in the arrow category, then $f'$ is a trivial fibration.
\item For any map $f:Y\ra X$ and any trivial fibration $A\ra X$,  the pullback $f^*A \ra Y$ is a trivial fibration.  
...
%}
%\end{equation}
\item For any map $f:Y\ra X$ and any trivial fibration $A\ra Y$,  the pushforward $f_*A \ra X$ is a trivial fibration.  
...
We now specify a second weak factorization system, with a restricted class of ``trivial'' cofibrations on the left, and an expanded class of right maps, the \emph{fibrations}. For comparison, we first recall the ``biased'' trivial-cofibration/fibration weak factorization system from \cite{GS}, which makes use of \emph{connections}, $$\vee,\wedge : \I\times\I \too \I,$$ on the cubes, which we do not assume (in \cite{AGH} it is shown that the fibrations of \cite{GS} agree with those specified in the ``logical style'' of \cite{CCHM,OP}).  
...
Condition \eqref{item:fibrant} above, which is of course a special case of \eqref{item:fibration}, says that evaluation at the generic point $\delta : 1\ra \I$, i.e.\ the map $X^\delta: X^\I \ra X$ constructed in the slice category $\cSet/\I$, is a trivial fibration.  
Condition \eqref{item:fibration} says that the pullback-hom of the generic point $\delta : 1\ra \I$ with $\I^*f$, constructed in the slice category $\cSet/\I$, is a trivial fibration.  The latter can be reformulated as follows.
...
\end{lemma}
\begin{proof}
This is clear, since fibrations are the right class of a weak factorization system, but note that the algebraic structure indicated in  \eqref{diagram:frobenius1} is also stable under pullback, since the $+$-algebras are pullback stable.
\end{proof}
...
\end{lemma}
\begin{proof}
Again for maps in the right class of a weak factorization system this is immediate.  But let us see how the \emph{fibration structures} also compose.  We have the following diagram for the fibration structures on $B\ra A$ and $A\ra X$ (with obvious notation).
...
\end{equation}
Pulling back $B\ra A$ in two steps we therefore obtain the interpolant $B_{\epsilon_X} $,
...
Our construction of $\dot{\U}\ra \U$ makes use of a new analysis of the well-known Hofmann-Streicher universe in a category $\widehat{\bbC} = [\op{\bbC}, \Set]$ of presheaves on a small category~$\bbC$, which was used in \cite{HS:1997} to interpret dependent type theory (but occurs already in \cite{SGA4}). See \cite{awodey:HSuniverse} for further details. 
...
\begin{itemize}
%
\item In contrast to \cite{HS:1997}, in \eqref{eq:universeob}  we take the underlying \emph{set of objects} of the functor category $\widehat{\bbC/_c}=[\op{\bbC/_c}, \Set]$.
%\psh{\bbC/_c}$.

\item Following \cite{HS:1997}, \eqref{eq:universeel} adopts the ``categories with families'' point of view in specifying a presheaf on the category of elements $\int_{\bbC}U$, rather than an arrow $E\to U$ in $\widehat{\bbC}$, using the equivalence
\begin{equation}\label{eq:elements}\textstyle
\widehat{\int_{\bbC}U}\ \simeq\ \widehat{\bbC}/_U\,.
\end{equation}
This equivalence is mediated by setting
\[
...
The presheaf $U$, which is not small, is then regarded as the Grothendieck universe $u$ ``lifted'' from $\Set$ to $[\op{\bbC}, \Set]$ \cite{Streicher:lifting}.  We first analyse this specification of $(U, {\mathsf{E}l})$ from a different perspective, in order to establish its basic property as a classifier for small families in $\widehat\bbC$. 
...
For the terminal presheaf $1\in\widehat{\bbC}$ we have $\elem{1} \cong\bbC$, so for every $X\in\widehat{\bbC}$ there is a canonical projection  $\elem X \ra\bbC$, which is a discrete fibration.  It follows that for any map $Y\to X$ of presheaves, the associated map $\elem Y \to \elem X$ is also a discrete fibration. 
...
It then follows from Proposition \ref{prop:familyclassifier} that, if $A\ra X$ is small, then $+\mathsf{Alg}(A) \ra X$ is itself a pullback of the analogous object $+\mathsf{Alg}(\VV) \ra \V$ constructed from the universal small family $\VV\ra\V$, so there are two pullback squares:
...
In order to classify fibrations $A\fib X$, we proceed as for trivial fibrations by constructing an object $\Fib(A)\ra X$ of fibration structures on a map $A\ra X$ which, moreover, is stable under pullback.  We then apply the construction to the universal small family $\VV\ra\V$ to obtain a universal small fibration.  
...
\begin{enumerate}
\item First, pull the map $p_A : A\ra X$ back  to $\cSet/_\I$ by applying the functor $\I\times (-) : \cSet \to \cSet/_\I$.  We continue to write $p_A : A\ra X$ the resulting map over $\I$. 

\item Form the pullback-hom $\delta\Rightarrow p_A : A^\I \ra X^\I \times_X A$ of $p_A$ with the generic point $\delta : \I \to \I \times \I$ over $\I$, as indicated in the following diagram.
...
\item A fibration structure on $p_A : A\ra X$ is then a relative +-algebra structure on the map $\delta\Rightarrow p_A$ in the slice category over the codomain $X^\I \times_X A$ (and that in the slice over $\I$). 

In order to construct the object $\Fib(A)\ra X$ classifying such structures, let us first relabel the objects and arrows in diagram \eqref{diagram:fibU1} as follows:
\begin{align*}
p &:= p_A\\
...
\item As in the construction \eqref{diag:plualgstru}, there is an object $+\mathsf{Alg}(\epsilon^A)$ over  $A_\epsilon$ of +-algebra structures on $\epsilon^A$, the sections of which correspond uniquely to +-algebra structures on $\epsilon^A$ (and thus fibration structures on $A$).
...
\item Sections of $+\mathsf{Alg}(\epsilon^A)\ra A_\epsilon$ then correspond to sections of its push-forward along $p_\epsilon$, which we shall call $F_A$: 
\[
...
\item One might now think of taking another pushforward of $F_A \ra X^\I$ along $\epsilon : X^\I\ra X$ to get the object $\Fib(A) \ra X$ that we seek, but unfortunately, this would not be stable under pullback along arbitrary maps $Y\ra X$, because $\epsilon : X^\I\ra X$ is not stable in that way.  Instead we use the \emph{root} functor of Proposition \eqref{prop:Itiny}, i.e.\ the right adjoint of the pathspace, $(-)^\I \dashv (-)_\I$.

Let $f : F_A \ra X^\I$ be the map indicated in \eqref{diagram:fibU5}, and $\eta_X : X \ra (X^\I)_\I$ the unit of the root adjunction.  Then define $\Fib(A)\ra X$ by 
$$ \Fib(A) :=  \eta^*{f_\I}$$
...
\item\label{item:laststep} Finally, recall that we are still working in the slice $\cSet/\I$ and need to get back to $\cSet$ by applying the functor $\I_*: \cSet/\I \ra \cSet$.  Call the map $\Fib(A)\ra X$ constructed over $\I$ in the last step $\Fib(A)_i \ra \I^{*}X$, and apply $\I_*$ to get,
\[
...
\end{equation}
It then follows directly from the adjunction $\I^*\dashv \I_*$ that sections of $\Fib(A)\ra X$ correspond bijectively to sections of $\Fib(A)_i \ra \I^{*}X$ over~$\I$.
\end{enumerate}

Now for the universal family $\VV\to\V$, let $\Fib = \Fib(\VV)$, which comes with its projection $\Fib(\VV) \ra \V$, and define the universal small fibration $\FFib\fib\Fib$ by pulling back the universal small family,
...
The map $$\FFib\ra\Fib$$ just constructed is a \emph{universal small fibration}: every small fibration $A \fib X$ is a pullback of $\FFib \fib \Fib$ along a canonically determined classifying map $X\ra \Fib$.
...
The relevant parts of the construction diagram \eqref{diagram:fibU5} are repeated below,
...
\end{equation}
On the left side we repeat the construction with $B \ra Y$ in place of $A\ra X$.  
...
Thus we can indeed use $\Fib := \Fib(\VV)$ to define the universal small fibration $\FFib\ra\Fib$ as in \eqref{diagram:universalfib}, and the proof can conclude just as that for proposition~\ref{prop:classTFib}.  
\end{proof}
...
Write $\UU \fib \U$ for the universal small fibration $\FFib\fib\Fib$ just constructed in proposition~\ref{prop:UniversalFib}.
\end{definition}
...
\end{lemma}
%
%\noindent Note that the condition on cofibrations holds for the locally decidable monos.
%
\begin{proof}
...
In light of Lemma \ref{lemma:Fibweakprop} we assume a final axiom on cofibrations:
...
%\begin{proposition}\label{prop:classifyFib}
%There is a \emph{universal small fibration}  $\UU\fib\U$.  Every small fibration $E \fib X$ is a pullback of $\UU\fib\U$ along a classifying map $X\to \U$.
%\begin{equation}\label{diagram:classifyfib}
%\begin{tikzcd}
%E \ar[d, two heads] \ar[r]  \pbmark & \UU\ar[d, two heads]\\
%X \ar[r] & \U
%\end{tikzcd}
%\end{equation}
%\end{proposition}
%
%\begin{proof}
%Take $\U = \Fib(\VV) \to \V$, as above, and define $\UU\fib\U$ by pulling back the universal small family.
%\[
%\begin{tikzcd}
%\UU \ar[d,two heads] \ar[r]  \pbmark & \VV \ar[d]\\
%\U \ar[r] & \V
%\end{tikzcd}
%\]
%Consider the following diagram in which all squares are pullbacks, the outer one classifying the family $E\to X$ and the lower one from the stability of $\Fib$.
%%
%\begin{equation}\label{diagram:classifyfib2}
%\begin{tikzcd}
%& E \ar[ddd] \ar[rrr]   \ar[rrd, dotted] &&& \VV \ar[ddd]\\
%& && \UU \ar[d] \ar[ru]  &\\
%\Fib(E) \ar[rd] \ar[rrr]  &&& \U \ar[rd] &\\
%&X \ar[rrr] \ar[lu, bend left, dotted] \ar[rru, dotted,swap, "e"] &&& \V
%\end{tikzcd}
%\end{equation}
%A fibration structure on $E\to X$ corresponds to a factorization $e$ as indicated, which in turn determines a pullback square of the required form.  Note that  $\UU\to\U$ has a canonical fibration structure,
%%. Indeed, consider the following diagram, in which both squares are pullbacks.
%%\begin{equation}\label{diagram:fibisfib}
%%\xymatrix{
%%\TTFib \ar[d]_{p_t} \ar[r]  & \VV \ar[d]\\
%%\TFib \ar[r] & \V\\
...
%%}
%%\end{equation}
%%$\TFib(\VV)$ is the object of trivial fibration structures on $\VV\ra\V$, and its pullback $\TFib(\TTFib)$ is therefore the object of trivial fibration structures on $p_t : \TTFib\ra\TFib$.  Thus we seek a section of $\TFib(\TTFib) \ra \TFib$.  But recall that $\TFib = \TFib(\VV)$ by definition, so the lower pullback square is the pullback of $\TFib(\VV)\ra \V$ against itself, which does indeed have a distinguished section, 
%namely the diagonal
%\[
%\U \to \Fib(\UU) \cong \U\times_\V \U.
%\]
%\end{proof}
%
...
\end{equation}
there is a fibration $A\onto C$ and a weak equivalence $w: A \simeq B$ over $C$ that pulls back along $c:C' \mono C$ to $w'$, so that $c^*w = w'$.
...
Conversely, the FEP also implies the fibrancy of $\U$, given the Realignment Lemma \ref{lemma:realignmentforfibrations}.
...
\item the weak equivalences $\WW$ are the maps $w : X\ra Y$  for which internal precomposition $K^w : K^Y \to K^X$ is bijective on connected components for every fibrant object $K$.
\end{enumerate}
\end{theorem}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Appendix: Axioms for cofibrations}\label{appendix:Cofibrations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The axioms for the \emph{cofibrations} $c : A \mono B$ in cartesian cubical sets, may be restated as follows.
\begin{enumerate}
%\item[(C0)] The map $0\to C$ is always a cofibration.
\item[(C1)] All isomorphisms are cofibrations.
\item[(C2)] The composite of two cofibrations is a cofibration.
\item[(C3)] Any pullback of a cofibration is a cofibration.
\item[(C4)] There is a terminal object of the form $t:1\mono \Phi$ in the category of cofibrations and cartesian squares.
\item[(C5)] The diagonal map $\delta : \I\ra\I\times\I$ is a cofibration.
\item[(C6)] The join of two cofibrant subobjects is a cofibration.
\end{enumerate}

\begin{proposition}
The locally decidable subobjects satisfy the axioms.
\end{proposition}
\begin{proof}
...
%Rather: initial among cubical qmc’s
%Universe
