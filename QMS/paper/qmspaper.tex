...
Now let us use the EEP, Proposition \ref{prop:EEP}, to show that $\U$ is fibrant. We shall need the following two lemmas.

\begin{lemma}\label{lemma:Uintheslice}
The base change $\I^*:\cSet\to\cSet/\I$ takes the universal fibration $\UU\to\U$ in $\cSet$  to a universal fibration in the slice category $\cSet/\I$.
\end{lemma}
...
The axioms (C0)--(C8) for the \emph{cofibrations} $c : A \mono B$ in cartesian cubical sets, may be restated equivalently as follows.
\begin{enumerate}
\item[(C1)] All isomorphisms are cofibrations.
\item[(C2)] The composite of two cofibrations is a cofibration.
\item[(C3)] Any pullback of a cofibration is a cofibration.
