...
\item[(C6)] Cofibrations are closed under joins $A\vee B \mono C$ of subobjects $A, B \mono C$ of any object $C$.
\end{enumerate}
%
...
It now ``only'' remains to show that the weak equivalences $\WW$ satisfy the 3-for-2 axiom in order to finish the proof that $(\mathcal{C}, \mathcal{W}, \mathcal{F})$ is a model structure.  Perhaps surprisingly, this requires a considerable amount of work, which will occupy the rest of these lectures!
We begin by following roughly the approach of \cite{Joyal}, as follows. We expect the weak equivalences between fibrant objects to be exactly the \emph{homotopy equivalences} (Definition \ref{def:homotopyequivalence}), which can be seen directly to satisfy 3-for-2.   So we reduce to this case, using the fact that $K^X$ is fibrant whenever $K$ is.  It should suffice, namely, to show that weak equivalences $w : X\to Y$ induce homotopy equivalences $K^w : K^Y \simeq K^X$ for fibrant $K$, and that any map with this property is a weak equivalence.   Such maps are termed \emph{weak homotopy equivalences}  (Definition \ref{def:WHE}), and our task is therefore reduced to showing that a map is a weak equivalence if and only if it is a weak homotopy equivalence.
...
In virtue of Lemma \ref{lemma:WHE342} it suffices to show that a map is a weak equivalence if and only if it is a weak homotopy equivalence.   This will be shown by splitting into four cases: a cofibration/fibration is a weak equivalence if/only if it is a weak homotopy equivalence.  The following will be useful thereby.
...
Any trivial cofibration $c : A \mono B$ is a weak homotopy equivalence.
\end{lemma}

\begin{proof}
Applying Lemma \ref{lem:WHEunwound}, condition (1) is immediate, and (2) results from the fact that $K^\I \fib K\times K$ is a fibration when $K$ is fibrant, since $\del : 1+1 \cof \I$ is a cofibration (by Remark \ref{rem:somecofibs}).
\end{proof}

\begin{lemma}\label{lemma:FibWEtoWHE}
Any trivial fibration $p : Y \onto X$  is a weak homotopy equivalence.
\end{lemma}
\begin{proof}
By Corollary \ref{cor:plusalgprops}, the trivial fibration $p : Y \onto X$ has a section $s: X\to Y$. 
Consider the following lifting problem:
\begin{equation*}%\label{diagram:extensionforX}
\xymatrix{
Y+Y \ar@{>->}[d]_{[\iota_0, \iota_1]} \ar[r]^-{[sp, 1]}  & Y\ar@{>>}[d]^{p} \\
\I\times Y \ar[r]_{p\pi_2} & X
}
\end{equation*}
A diagonal filler is then a homotopy $\vartheta : sp \sim 1_Y$. 
Thus $p$ is a homotopy equivalence, and therefore a weak homotopy equivalence by Lemma \ref{lemma:HEisWHE}.
\end{proof}

\begin{corollary}
Any weak equivalence $e : X \simeq Y$ is a weak homotopy equivalence, since $e$ factors into a trivial cofibration followed by a trivial fibration, each of which is a weak homotopy equivalence, and these are closed under composition by Lemma \ref{lemma:WHE342}. 
\end{corollary}

\begin{lemma}\label{FibHETFib}
If $K$ is fibrant, then any fibration $f : Y \onto K$ that is a homotopy equivalence is a weak equivalence.
\end{lemma}
\begin{proof}
...
Since $f$ is a homotopy equivalence, it has a quasi-inverse $s:X\ra Y$ with $\vartheta : fs\sim 1_K$, which we claim can be corrected to a section $s' : K\ra Y$. 
Indeed, consider 
\begin{equation*}%\label{diagram:extensionforX}
\xymatrix{
...
If $K$ is fibrant, then any fibration $f : Y \onto K$ that is a weak homotopy equivalence is a weak  equivalence.
\end{lemma}
\begin{proof}
Since $K$ is fibrant, so is $Y$, and since $f$ is a weak homotopy equivalence, by lemma \ref{lem:WHEunwound}(1) there is then a map $s : K\ra Y$ and a homotopy $\theta: sf \sim 1_Y$.  Postcomposing with $f$ gives a homotopy $f\vartheta: fsf \sim f$, forming the outer commutative square in
\[
\xymatrix{
...
A cofibration $ c : A \mono B$ weak homotopy equivalence lifts against all fibrations $f : Y\onto K$ with fibrant codomain.
\end{lemma}
%
\begin{proof}
...
To complete the proof that a cofibration weak homotopy equivalence is a weak equivalence, we shall use the following \emph{fibration extension property}, the proof of which is deferred to section \ref{sec:FEP}.
...
Assuming the fibration extension property,  a cofibration $c : A \mono B$ weak homotopy equivalence is a weak equivalence.
\end{corollary}
...
Assuming the fibration extension property, a map $ f : X \to Y$ is a weak homotopy equivalence if and only if it is a weak equivalence.  The weak equivalences $\mathcal{W}$ then satisfy the 3-for-2 condition.
\end{proposition}
\begin{proof}
...
Our results can now be summarized as follows.

\begin{theorem}\label{theorem:QMSmodFEP}
Assume the fibration weak factorization system of Definition \ref{def:FibWFSclasses} satisfies the fibration extension property of Definition \ref{def:fibextreplace} (as will be shown  in Corollary \ref{cor:FEP} to obtain). 
Then the weak equivalences $\WW$ have the 3-for-2 property, and so by Proposition \ref{prop:FWC}, the classes $(\CC,\WW,\FF)$ form a Quillen model structure on the category $\cSet$ of cubical sets.
Moreover, the weak equivalences can be characterized as those maps $f: X\ra Y$ for which $K^f : K^Y \to K^X$ is bijective on connected components whenever $K$ is fibrant.
\end{theorem}
