...
\title{On the cubical model of homotopy type theory\thanks{
...
\noindent It follows that there is a cubical model of homotopy type theory in which the identity type of a type $A$ is the path-object $A^\I$, a choice with some advantages.
...
Examples include (i) Groupoids (or categories), with $\R$ = isofibrations and $\L$ = injective equivalences; (ii) Simplicial sets, with $\R$ = Kan fibrations and $\L$ = acyclic cofibrations.  A Quillen model structure on a category involves two such interrelated weak factorization systems, providing many examples and the basic homotopical intuition. In a WFS, we may think of the \R-maps as ``fibrations'', i.e.\ good families of objects indexed by the codomain.  The basic idea of the homotopy interpretation is to use these as the dependent types.
...
\begin{equation}\label{rule:idform}\tag{$\Id$-Formation}
\begin{prooftree}
A\ \type 
\justifies
...
\begin{equation}\label{rule:idintro}\tag{$\Id$-Introduction}
...
\]
%
The data above the line in $\Id$-elimination are interpreted as a commutative square as on the outside of the diagram:
\[
\xymatrix{
A \ar[d]_{\refl} \ar[r]^b & B \ar[d] \\
\id{A} \ar[r] \ar@{..>}[ru]_{\mathsf{J}} & \id{A} 
}
\]
Since $\refl$ is an \L-map, and $B\arr \id{A}$ is an \R-map (as the interpretation of a dependent type), there is a diagonal filler $\mathsf{J}$ as indicated.  Commuting of the lower triangle is just the conclusion of the \Id-elimination rule.  The commuting of the upper triangle is exactly the $\mathsf{J}$-computation rule.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The cubical model}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Out goal is to make an algebraic weak factorization system on the cubical sets, but let us first recall \emph{why} the cubical sets is a good setting for a model of \Id-types.  The basic examples of WFSs and homotopy are topological spaces and simplicial sets, but these models are not entirely satisfactory, at least from a logical point of view.  In particular, they seem to lack the combinatorial character that would make them \emph{constructive}.  The cubical approach seems to be better suited to giving such a constructive treatment, as has recently been discovered by T.~Coquand and his coworkers.  It is worth noting in this connection that the goal here is \emph{not} to give a constructive interpretation of the univalence axiom, as Coquand et al.\ have done, but only to investigate the cubical model as arizing from an algebraic weak factorization system.

By way of motivation for using cubical sets, consider the important factorization of the diagonal for an object $A$,
...
\end{equation}
We regard $PA$ as an abstract  ``pathspace'' $A^\I$.  If there is an ``interval object'' $\I$ with two points $1\to\I$, then exponentiating $A$ by the structure:
\[
\xymatrix{
& \ar[ld] \I  & \\
1 && \ar[ll] \ar[ul] 1+1\,
...
\]
will result in such a factorization of the form \eqref{diag:pathfact} (although unrelated to any WFS).

The idea that identity proofs are paths and that $\id{A} = A^\I$ is also supported by the fact that dependent types have the path-lifting property.  Given $a_0,a_1:A$  and $p:\id{A}(a_0,a_1)$ and $\types{x:A}{B(x)}$  and $b_0:B(a_0)$, there is a distinguished $b_1:B(a_1)$, called the \emph{transport} of $b_0$ along $p$ and denoted $p_*(b_0)$.  Indeed, there is even a path $\tilde{p} : \id{B}(b_0,b_1)$, where $B=\sum_{x:A}B(x)$, such that $\pi_1(\tilde{p})=p$.
\[
\xymatrix{
...
\]
%
Note that this says exactly that the endpoint inclusion $0 : 1\to \I$ has the left-lifting property with respect to the dependent projection $\pi_1:B\to A$,
\[
\xymatrix{
1 \ar[d]_{0} \ar[r]^{b_0} & B \ar[d]^{\pi_1} \\
...
An important consequence of the proposed stipulation $\id{A} = A^\I$ is that the iterated or``higher'' \Id-types can be represented as ``cube types'',
\begin{align*}
\id{A}\ &=\ A^\I \\
...
\cSet\ =\ \psh{\C_c}
\]
on the category $\C_c$ of \emph{(cartesian) cubes}, defined equivalently as:
\begin{enumerate}
...
\end{enumerate}
The equivalence of these three specifications of $\C_c$ is an easy application of Lawvere duality.
\end{definition}
...
This combinatorial description of the pathobject $X^\I$ as a shift in dimension has consequences for the model of type theory, in particular it and leads certain equations holding strictly that are usually satisfied only weakly or ``up to homotopy''.

%%%%%%%%%%%%%%%%%
\subsection{Box filling}
%%%%%%%%%%%%%%%%%

We now turn to the determination of the right maps for our AWFS.    We know that they should at least have the path-lifting property, since that is forced by the rules of type theory.  We also know that for any type $A$, the interpretation of $\Id{A}$ should be a right map into $A\times A$, since it is a dependent type. Finally, we want to use the pathobject $A^\I$ as the interpretation of $\Id{A}$ in view of the foregoing considerations. These constraints lead us directly to the box-filling condition.
...
\]
The index $j = 1, 2$ is the coordinate in which the box is open, while $e = 0,1$ indicates which face of the box is missing, bottom or top.   Because we are in the symmetric situation, where the product $\I\times \I$ can be twisted, it  will suffice to  consider only  boxes that are open in the first coordinate, since the others can be constructed from those.  Thus we may omit the index $j$, writing $\sqcup^2_e$ for $\sqcup^2_{1,e}$. Moreover, let us write $\sqcup^2 = {\sqcup}^2_1$ and $\sqcap^2 = {\sqcup}^2_0$.  The \emph{(upper and lower) open $n$-boxes} $\sqcup^n, \sqcap^n \mono \I^n$ are defined analogously.
...
Indeed, the foregoing can be generalized as follows:
...
\begin{remark}
Since the composition of two maps with $n$-box filling clearly also has $n$-box filling, if an object $A$ has $(n+1)$-box filling then its pathobject $A^\I$ has only $n$-box filling.  Thus if we want all types to have \Id-types, and if we have $\Sigma$-types so that we can form $\id{A} = \sum_{x,y:A}\id{A}(x,y)$,  then we are led to take as the types (at most) those cubical sets that are Kan complexes, and as the dependent types the Kan fibrations.  It seems remarkable that the constraints imposed by modelling \Id-types have led us directly to the definition of Kan fibration and Kan complex!
\end{remark}

\subsection{Uniformity}

We could now use Quillen's small object argument to make a WFS in which the \R-maps are the Kan fibrations by taking as a generating set of left maps all the open box inclusions.  Since we want an \emph{algebraic} WFS, however, we shall instead use a refinement of the SOA due to R.~Garner, involving an algebraic notion of fibration.

\begin{definition}[cf.~\cite{BCH}]  A cubical set $X$ is a \emph{uniform Kan complex} if it has the following structure:
\begin{enumerate}
\item  For each open box $\sqcup^n \mono \I^n$, each $k\geq 1$, and each map $b: \I^k \times\sqcup^n\to X$, there is given an extension $\bar{b} : \I^k\times\I^n \to X$ along the product map $\I^k\times\sqcup^n \mono \I^k\times\I^n$.
...
\end{equation}
\item The chosen extensions $\bar{b}$ are natural in $\I^k$, in the sense that for each map of cubes $f : \I^j \to \I^k$, one has
\[
...
The uniform Kan condition turns the box filling \emph{property} of an object $X$ into an explicitly given \emph{structure} $(X, \overline{(-)})$ on the object, namely a natural choice of fillers $\bar{b}$ for all (generalized) open boxes $b : \I^k\times \sqcup^n \to X$.  The generalization to maps is straightforward:
...
\item For every (outer) square of the form
\begin{equation}%\label{diagram:boxfillfib}
\xymatrix{
\I^k \times \sqcup^n \ar[d]_{\I^k\times i^n} \ar[r]^-{b} & Y \ar[d]^{f} \\
\I^k \times \I^n \ar[r]_-{a} \ar@{.>}[ru]|-{c} & X
}
\end{equation}
there is given a diagonal filler $c = \overline{(a,b)}$.

\item These chosen fillers are moreover natural in $\I^k$, in the sense that for any $f : \I^j \to \I^k$, one has
\[
...
\begin{theorem}
There is an algebraic weak factorization system on $\cSet$ for which the $\RR$-algebras are exactly the uniform Kan fibrations.
\end{theorem}

This is now a direct application of Garner's small object argument. We will nonetheless sketch the details of the argument in the next section, because some modifications of it will be required thereafter. 

\section{Constructing the (first) algebraic weak factorization system}
